<template>
  <body>
        <div class="hidden py-12 dark:bg-black bg-gray-700 transition duration-150 ease-in-out z-10 absolute top-0 right-0 bottom-0 left-0" ref="modalSignup2" id="modal">
...
export default {
  ...
  methods: {
    modalHandler(val) {
      let modal = this.$refs.modalSignup2;
      if (val) {
        this.fadeIn(modal);
      } else {
        this.fadeOut(modal);
