<template>
    ...
    <div role="alert" class="container mx-auto w-11/12 md:w-2/3 max-w-lg">
      <div class="flex items-center justify-center py-8 px-4">
        <div class="relative md:w-80 rounded shadow-lg p-6 dark:bg-gray-800 bg-white">
          <h1 class="text-lg font-bold leading-none dark:text-gray-100 text-gray-800">
            We need your consent
          </h1>
          <p class="text-sm leading-5 pt-6 dark:text-gray-100 text-gray-600">
          ...
          <div class="flex items-center justify-between mt-4 border-gray-200 border rounded-lg px-4 py-4 cursor-pointer">
            <p class="text-base font-medium leading-4 dark:text-gray-100 text-gray-800">
          ...
          <div class="py-6 flex items-center">
            <div class="dark:bg-gray-800 bg-white rounded-full w-5 h-5 flex flex-shrink-0 justify-center items-center relative">
              <input type="radio" name="radio" class="checkbox appearance-none focus:outline-none rounded-full border-gray-400 absolute cursor-pointer w-full h-full border checked:border-none" @click="terms = !terms">
              <div class="check-icon hidden border border-indigo-700 flex items-center justify-center rounded-full w-full h-full z-1">
                <div class="bg-indigo-700 rounded-full w-2 h-2" />
              </div>
            </div>
            <p class="ml-3 text-base font-medium leading-4 dark:text-gray-100 text-gray-800">
              I read the documentation
            </p>
          </div>
          <button :disabled="terms" class="text-sm font-semibold leading-6 bg-indigo-700 rounded py-2 w-full hover:opacity-90 focus:outline-none text-white">
            Accept
          </button>

          <div class="cursor-pointer absolute top-0 right-0 mt-4 mr-5 dark:text-gray-100 text-gray-400 hover: dark:text-gray-100 text-gray-600 transition duration-150 ease-in-out" @click="modalHandler(false)">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              aria-label="Close"
  ...
  <div id="button" class="w-full flex justify-center py-12">
    <button class="focus:outline-none mx-auto cursor-pointer transition duration-150 ease-in-out hover:bg-indigo-600 bg-indigo-700 rounded text-white px-4 sm:px-8 py-2 text-xs sm:text-sm" @click="modalHandler(true)">
      Cookie consent
    </button>
  </div>
</template>
