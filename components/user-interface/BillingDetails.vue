<template>
  <div id="modalBilling" class="py-12 bg-gray-700 transition duration-150 ease-in-out z-10 absolute top-0 right-0 bottom-0 left-0 hidden">
    <div role="alert" class="container mx-auto w-11/12 md:w-2/3 max-w-lg">
      <div class="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">
        ...
        <div class="flex items-center justify-start w-full">
          ...
          <button class="focus:outline-none ml-3 bg-gray-100 transition duration-150 text-gray-600 ease-in-out hover:border-gray-400 hover:bg-gray-300 border rounded px-8 py-2 text-sm" @click="modalHandler()">
            Cancel
          </button>
        </div>
...
export default {
  methods: {
    modalHandler (val) {
      const modal = document.getElementById('modalBilling')
      if (val) {
        this.fadeIn(modal)
      } else {
        this.fadeOut(modal)
