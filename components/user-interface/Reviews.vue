  ...
  <div class="bg-gray-300 min-h-screen">
    <div class="flex items-center justify-center py-8 px-4">
      <div class="bg-white md:w-96 rounded px-5 py-5 w-full shadow">
        <p class="text-lg font-bold leading-none text-gray-800">
          Reviews
        </p>
        <div v-for="(review, idx) in reviews" :key="idx" class="mt-6">
          <div class="flex items-start justify-between">
            <div class="flex items-start">
              <img class="w-8 h-8 rounded-full shadow" :src="review.img" alt="profile">
              <div class="ml-3">
                <p class="text-sm leading-4 text-gray-800 font-semibold">
                  {{ review.name }}
                </p>
                <p class="text-xs leading-3 text-gray-600 mt-1">
                  {{ review.position }}
                </p>
                <p class="text-sm leading-4 text-gray-800 my-3">
                  {{ review.comment }}
                </p>
                <p class="text-xs leading-3 text-gray-500">
                  {{ review.day }} days ago
                </p>
              </div>
            </div>
            <div class="flex items-center">
              ...
              <p class="text-sm leading-none text-yellow-500 ml-1">
                {{ review.stars }}
              </p>
            </div>
          </div>
