    ...
    <div class="md:w-96 rounded shadow-lg py-4 px-5 bg-white">
      <div class="flex items-start justify-between">
        <div class="flex items-center mb-4 lg:mb-0 mr-10">
          ...
          <div>
            <p class="text-sm font-bold leading-4 text-gray-800">
              {{ name }}
            </p>
            <p class="text-xs leading-3 text-gray-500 pt-2">
              {{ title }}
            </p>
          </div>
        </div>
      ...
      <p class="text-xs font-medium leading-3 pt-5 text-indigo-700">
        CONNECTIONS
      </p>
      <div class="flex items-center pt-2 pb-2.5">
        <div v-for="(connection, idx) in connections" :key="idx" class="w-6 h-6 mr-1 shadow rounded-full">
          <img alt="img-1" :src="connection.img" class="w-full h-full object-cover object-center rounded-full">
        </div>
      </div>
      <div class="flex items-end justify-end">
        <button class="text-xs font-medium leading-3 text-white py-3 px-4 rounded bg-indigo-700 focus:outline-none hover:opacity-90">
          Send a message
        </button>
      </div>
    </div>
