...
<template>
  <!-- Navigation starts -->
  <nav class="w-full mx-auto bg-gray-800 shadow z-20 relative m-0">
      ...
      <ul class="flex items-center justify-center h-full">
        <li
          ...
          :class="route.fullPath.includes(page.slug)
            ? 'cursor-pointer h-full xl:flex items-center text-sm text-yellow-300 tracking-normal hidden mr-10 min-w-[50px]'
            : 'hover:text-yellow-300 cursor-pointer h-full xl:flex hidden items-center text-sm text-white mr-10 tracking-normal relative  min-w-[50px]'
          "
        ...
        <li class="mx-0 xl:mr-12 cursor-pointer h-12">
          <img src="/logo.png" alt="Drew MacGibbon Development (DM Development)" class="h-full" @click="navigateTo(pages[0].slug)">
        </li>
        <li
