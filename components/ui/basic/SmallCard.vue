  ...
  <div class="bg-gray-300 min-h-screen">
    <div class="flex items-center justify-center py-8 px-4">
      <div class="md:w-96 rounded shadow-lg py-4 px-5 bg-white">
        <h1 class="text-lg font-bold leading-4 text-gray-800">
          {{ title }}
        </h1>
        <h2 class="pt-2 text-xs leading-3 text-gray-600">
          {{ client }}
        </h2>
        ...
        <div class="flex items-center pt-4">
          <div v-for="(attendee, idx) in attendees" :key="idx" class="w-6 h-6 mr-1 shadow rounded-full">
            <img alt="img-1" :src="attendee.img" class="w-full h-full object-cover object-center rounded-full">
          </div>
        ...
        <div class="mt-6">
          <p class="text-sm font-semibold text-indigo-700 leading-none text-right pb-1">
            73%
          </p>
        </div>
        <div class="w-full h-2 relative bg-gray-200 rounded-full">
          <div class="h-2 w-60 bg-gradient-to-r from-blue-700 via-indigo-600 to-purple-600 rounded-full" />
        </div>
        <button class="focus:outline-none text-indigo-700 hover:opacity-50 bg-gray-100 text-sm font-medium py-3 w-full rounded mt-5">
...
const title = 'UX optimization and UI design'
const client = 'Client: Imaginary Company LLC'
